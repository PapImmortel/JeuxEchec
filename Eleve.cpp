...
struct _plateau {
    ...
    bool getPositionPiece(V2 pos) { return positionPiece[(8 - pos.y - 1) * 8 + pos.x]; }
};
struct _Piece {
    int couleur;
    V2 pos;
...
struct _Fou : public _Piece {
    ...
    string fouNoir =
        "[                   KKKKKKKKK                                                ]"
        "[                 KKBBBBBKSSSKKKKK                 KKKKKK                    ]"
        "[                KBBBBBBBBKSKBBBBKKK           KKKKKKKKKKKK                  ]"
        "[               KBBBKBBBBBBKBBBBBKSSK      KKKKKKKKKKKKKKKKK                 ]"
        "[               KBBKSKBBBBBBBBBBBBKKKK   KKSSKKKKKKKKKKKKKSSK                ]"
        "[              KBBBKSSKBBBBBBBBBBBBBBK  KKKKKKKKKKKKSKKKKKKKK                ]"
        "[              KBBBKSKBBBBBBBBBKBBBBBBKKKKKKKKKKKKKSKKKKKKKKKK               ]"
        "[             KBBBBBKBBBBBBBBBKSKBBBBBKKKKKKKKKKKKKKKKKKKKKKKK               ]"
        "[             KBBBBBBBBBBBBBBKSSSKBBBBKKKKKKKKKKKKKKKKKKKKKKKKK              ]"
        "[             KBBBBBBBBBBBBBBBKKKBBBBBKKKKSKKKKKKKKKKKKKKSKKKKK              ]"
        "[            KBBBBBBBBBBKBBBBBBBBBBBBBKKKKSKKKKKKKKKKKKKKSKKKKK              ]"
        "[            KBBBBBBBBBKSKBBBBBBBBBBBBKKKKKKKKKKKKKKKKKKKKKKKKKK             ]"
        "[            KKKBBBBBKKKSSKBBBBBBBBBBBBKKKKKKKKSKKKKKKKKKKKKKKKK             ]"
        "[            KSSKBBKK   KKBBBBBBBKKKBBBKKKKKKKKKSKKKKKKKKKKKKKKK             ]"
        "[            KKSKBK       KBBBBBKSSSKBBKKKKKKKKKKKKKKK  KKKKKKSK             ]"
        "[            KBKKBK       KBBBBBBKSKBBBKKKSKKKKKKKKKK      KKKKK             ]"
        "[            KBBBBK       KBBBBBKKKKKKKKKKKKKKKKKKKK       KKKKK             ]"
        "[             KBBK         KBBKKSSSSSSSKSSSSSSKKKKKK        KKK              ]"
        "[             KBK          KBKSSSSSSSSSSSSSSSSSSKKK          K               ]"
        "[              K            KKSSSSSSSSSSSSSSSSSSKK          KOK              ]"
        "[             KOK           KKSKKKKKSSSSSSKKKKKSK           KOK              ]"
        "[            KOOOK           KSKKKKKKSSSSKKKKKKSK            K               ]"
        "[             KKK            KKKKKKKKSSSSKKKKKKSK                            ]"
...
struct _Pion : public _Piece {
    string pionBlanc ="[        KKKKKK                     ]"          
        ...
        "[       KTKTTTTKK      KGSWKKKKKWGK ]"
        "[      KTTTKTTKTK      KGWK     KW4 ]"
        "[     KATTKTTKTK      KGWK       KSK]"
        
        ;

    string pionNoir = "[        KKKKKK                     ]"          
        "[       K445445KK                   ]"
        "[      K44545445KK                  ]"
        "[     K445554544KK                  ]"
        "[     KKK444KTTTK                   ]"
...
bool DeplacementPiece(_Piece Piece, V2 pNewPos){
    ...
    {
        if (((((vCoord + V2(0, 1)==pNewPos )|| (vCoord + V2(0, 2) == pNewPos && vCoord.y == 2)) && Piece.getCouleur() == 1 )|| (((vCoord + V2(0,-1) == pNewPos) || (vCoord + V2(0, -2) == pNewPos && vCoord.y == 7)) && Piece.getCouleur() == 2) || (vCoord + V2(1,1) == pNewPos || vCoord + V2(-1, 1) == pNewPos) || (vCoord + V2(1, -1) == pNewPos || vCoord + V2(-1, -1) == pNewPos)) && pNewPos.x > 0 && pNewPos.x < 9 && pNewPos.y > 0 && pNewPos.y < 9) {
